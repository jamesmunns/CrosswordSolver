...
def load_freq():
    ...
    with open('en_50k.txt', 'r') as fin:
        for lines in fin:
            sline = lines.split()
            freqlist.append( (freqword( sline[0].strip(), sline[1].strip().lower() ) ) )
    return freqlist
...
def main():
    ...
    while( True ):
        ...
        regexp = raw_input( "Regexp to match:\n>" ).strip()
        for word in worddict:
            if( is_regexp_match( regexp, word ) ):
                if word in freqlist:
...
def is_regexp_match( regexp, word ):
    pos_match = re.findall( regexp, word )
    for match in pos_match:
